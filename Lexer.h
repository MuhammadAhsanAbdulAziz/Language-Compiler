...
private:
    ...
    regex Number = regex("^[-+]?(?:[0-9]+.[0-9]*|.[0-9]+|[0-9]+)$");
...
public:
    ...
    void createToken()
    {
        ...
        while (getline(inputFile, strLine, '\n'))
        {
            ...
            {
                // stringstream checkTwice(str);
                // if (getline(checkTwice, str, ';') || getline(checkTwice, str, '{') || getline(checkTwice, str, '}') || getline(checkTwice, str, '(') || getline(checkTwice, str, '(') || getline(checkTwice, str, ':') || getline(checkTwice, str, '[') || getline(checkTwice, str, ']') || getline(checkTwice, str, '~'))
                // {
                    if (str == "``")
                    {
                        if (commentOn == 0)
